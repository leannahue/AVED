//
// TO RUN: gcc aved_quantized_no_cordic.c -o aved_quantized__no_cordic_exe -std=c99
//
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void write_accu_to_txt(const char *filename, unsigned int* accu, int accu_w, int accu_h) {
...
void cos_table(int deg, int *c) {
   int cos_theta;
      cos_theta = floor(cos(deg*MPI_real/180)*pow(2,14));
      if (cos_theta == 0) {
        *c = 1;
      }
      else {
      ...
      //      printf("theta: %d, cos_theta: %d\n",deg,cos_theta);
}

void sin_table(int deg, int *c) {
   int sin_theta;
      sin_theta = floor(sin(deg*MPI_real/180)*pow(2,14));
      if (sin_theta == 0) {
        *c = 1;
      }
      else {
...
int HoughTransform(unsigned char* img_data, int w, int h,  int hough_h, int accu_h, int accu_w, unsigned int* accu) { // hough_h is quantized
    ...
    printf("IMG_THRESHOLD: %d\n", IMG_THRESHOLD);

    // FILE* cos_file = fopen("cos_table.txt", "w");
    ...
    for(int y=0;y<h;y++)
    {
        for(int x=0;x<w;x++)
        {
            if( img_data[ (y*w) + x] > IMG_THRESHOLD )
            {
                ...
                for(int t=0;t<180;t++)
                {
                    ...
                    // fprintf (cos_file, "%d %d\n", t, cos_theta);

                    int theta = (t*571)/2;
                    cos_table(t, &cos_theta);
                    sin_table(t, &sin_theta);
...
int HoughGetLines(unsigned char* in_data, unsigned int* accu, int w, int h, int accu_h, int accu_w, struct pixel * out_data) {
    ...
    // Open file to print out threshold and finite lines outputs into text files
    FILE* threshold_file = fopen("hough_threshold_out.txt", "w"); // alternating lines of rho then theta
    FILE* lines_file = fopen("hough_lines_out.txt", "w");
    ...
    int accu_threshold = 200;
       printf("accu_threshold: %d \n", accu_threshold);
    ...
    for(int r=0;r<accu_h;r++)
    {
        for(int t=0;t<accu_w;t++)
        {
            if(accu[(r*accu_w) + t] >= accu_threshold)
            {
                ...
                // Print rho/theta onto txt file
                fprintf (threshold_file, "%d %d\n", r, t);
                ...
                // int theta = QUANTIZE_F(t*MPI/180);
                int theta = t*QUANTIZE_F(MPI) / 180;
                printf("theta: %d\n", theta);
                // cordic(theta, &sin_theta, &cos_theta);  // s is sine, c is cosine. Theta is in radian, quantized

                cos_table(t, &cos_theta);  // s is sine, c is cosine. Theta is in radian, quantized
                sin_table(t, &sin_theta);  // s is sine, c is cosine. Theta is in radian, quantized
...
int main(int argc, char *argv[]) {
    ...
    write_accu_to_txt("accu_c.txt", accu, accu_w, accu_h);
